...
//#include <vml2.0/libs/imgui/imgui_internal.h>
#include <vml2.0/libs/imgui-docking/imgui_internal.h>
#include <iostream>
#include <functional>
#include <climits>
#include <string.h>
...
namespace imgui_addons
{
    ...
    bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& valid_types)
    {
        ...
        if (ImGui::BeginPopup(label.c_str(), ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
...
//        if(show_inputbar_combobox && ( ImGui::GetCurrentFocusScope() == focus_scope_id || ImGui::GetCurrentContext()->ActiveIdIsAlive == input_id  ))
    ...
    {
        std::string str;
        size_t wsize=std::char_traits<wchar_t>::length(wchar_arr);
        str.resize(wsize);
        std::wcstombs(&str[0], wchar_arr, wsize);
        return str;
    }
