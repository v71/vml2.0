...
namespace vml
{
	namespace geo2d
	{
		...
		class PathFinder
		{
			...
			private:
				...
				void ParseLine(const std::string& line)
				{
					...
					int i  = strtol(strings[ 0].c_str(), &end, 10);
					int j  = strtol(strings[ 1].c_str(), &end, 10);

					unsigned int offset = i + Width * j;

                    Pa[offset].x = strtof(strings[2].c_str(), &end);
                    Pa[offset].y = strtof(strings[3].c_str(), &end);
                    Pa[offset].z = ZDepth;

                    Pb[offset].x = strtof(strings[5].c_str(), &end);
                    Pb[offset].y = strtof(strings[6].c_str(), &end);
                    Pb[offset].z = ZDepth;

                    Pc[offset].x = strtof(strings[8].c_str(), &end);
                    Pc[offset].y = strtof(strings[9].c_str(), &end);
                    Pc[offset].z = ZDepth;

                    Pd[offset].x = strtof(strings[11].c_str(), &end);
                    Pd[offset].y = strtof(strings[12].c_str(), &end);
                    Pd[offset].z = ZDepth;

					Data[offset] = strtol(strings[14].c_str(), &end, 10);
				...
				void LoadFile()
				{
					std::ifstream profilesFile(FileName);
					...
					if (profilesFile.is_open())
					{
						...
						std::getline(profilesFile, line);

						{
				//			std::string filename = line.substr(line.find(':') + 1, line.size());

				//			std::cout << line.substr(0, line.find(':')) << " : " << filename << std::endl;
						...
						{
							std::string wstring = line.substr(line.find(':') + 1, line.size());
							char* end;
							Width = (int)strtol(wstring.c_str(), &end, 10);

				//			std::cout << "Mask W = " << Width << std::endl;
						}
...

						{
							std::string wstring = line.substr(line.find(':') + 1, line.size());
							char* end;
							Height = (int)strtol(wstring.c_str(), &end, 10);

				//			std::cout << "Mask H = " << Height << std::endl;
						}
						
                        // read delta width ( width of 'cell' )
...

                        {
                            std::string wstring = line.substr(line.find(':') + 1, line.size());
                            char* end;
                            Dw = strtof(wstring.c_str(), &end);

                            //			std::cout << "Mask H = " << Height << std::endl;
                        }

                        // read delta height ( height of the 'cell' )
                        ...
                        BoundingBoxMin = Pa[0];
                        BoundingBoxMax = Pd[Width - 1 + (Height - 1) * Width];
					...
					{
						vml::os::Message::Error("Level : ","NavMask cannot load ' ", FileName.c_str()," '");
					}
                ...
                int GetAddress(const int i, const int j) const
                {
                    return i + j * Width;
                }
                ...
                int GetAddressFromPoint(const Node& p) const
                {
                ...
                bool FindNextFreeCell(const unsigned char* visited, Node& point)
                {
                    for (int j = 0; j < Height; ++j)
                    {
                        for (int i = 0; i < Width; ++i)
              ...
              //              std::cout << point.X << " " << point.Y << std::endl;
                ...
                int heuristic(const Node& start, const Node& dest)
                {
                ...
                int heuristic2(const Node& start, const Node& dest)
                {
		...
		public:
                ...
                float GetDw()                           const { return Dw; }
                float GetDh()                           const { return Dh; }
                const glm::vec3& GetBoundingBoxMin()    const { return BoundingBoxMin; }
                const glm::vec3& GetBoundingBoxMax()    const { return BoundingBoxMax; }
                int GetPathCount()                      const { return PAvgrOutPathCount; }
                const glm::vec3* GetPAvgrOut()          const { return PAvgrOut; }
                const glm::vec3* GetPAvgrDir()          const { return PAvgrDir; }
                const float *GetPAvgrInvSegmentLength() const { return PAvgrInvSegmentLength; }
                const float* GetPAvgrSegmentLength()    const { return PAvgrSegmentLength; }
                float GetZDepth()                       const { return ZDepth; }

                // -------------------------------------------------------------
                //

                bool IsPointInNavMesh(const glm::vec3& ra, const glm::vec3& rb) const
                {
                ...
                glm::vec3 GetCellCenterFromPoint(const glm::vec3& p) const
                {
                ...
                int GetCellAddressFromPoint(const glm::vec3& p) const
                {
                    int i = (int)((p.x - BoundingBoxMin.x) / Dw);
                    int j = (int)((p.y - BoundingBoxMin.y) / Dh);
                    if (i < 0 || j < 0 || i >= Width || j >= Height)
                ...
                glm::ivec2 GetCellIndicesFromPoint(const glm::vec3& p) const
                {
                    int i = (int)((p.x - BoundingBoxMin.x) / Dw);
                    int j = (int)((p.y - BoundingBoxMin.y) / Dh);
                    if (i < 0 || j < 0 || i >= Width || j >= Height)
                ...
                int GetCellIdFromIndices(int i, int j) const
                {
                    if (i < 0 || j < 0 || i >= Width || j >= Height)
                        vml::os::Message::Error("Pathfinder : ","index out of range");
                ...
                glm::vec3 GetCellCenterFromIndices(int i, int j) const
                {
                    if (i < 0 || j < 0 || i >= Width || j >= Height)
                        vml::os::Message::Error("Pathfinder : ", "index out of range");
                    int offset = i + j * Width;
                ...
                void GetCellVertices(int x, int y, glm::vec3& p0, glm::vec3& p1, glm::vec3& p2, glm::vec3& p3)
                {
                ...
                int FindPath(const glm::vec3 &origin,int ai, int aj, int bi, int bj)
                {
                ...
                //    }
                    ...
                    if (IdData[offseta] == IdData[offsetb])
                    {
                        ...
                        // add initial point to the array 

//                        PAvgrOut[0] = (Pa[offsetb] + Pd[offsetb]) * 0.5f;
                        PAvgrOut[0] = origin;
                ...
                glm::ivec2 FindNearestCellToAnotherCell(/*vml::views::View* view, */ int ai, int aj, int id)
                {
...
//
                ...
                void DrawBoundingBox(vml::views::View* view, const glm::vec4& col) const
                {
                    glm::vec3 b = glm::vec3(BoundingBoxMax.x, BoundingBoxMin.y, BoundingBoxMin.z);
                    glm::vec3 c = glm::vec3(BoundingBoxMin.x, BoundingBoxMax.y, BoundingBoxMin.z);
                    vml::DebugRender->DrawLine(view, BoundingBoxMin, b, col, true);
                    vml::DebugRender->DrawLine(view, b, BoundingBoxMax, col, true);
                    vml::DebugRender->DrawLine(view, BoundingBoxMax, c, col, true);
                    vml::DebugRender->DrawLine(view, c, BoundingBoxMin, col, true);
                }

                // ---------------------------------------------------------------
                //

                void DrawCell(vml::views::View* view, int x, int y, const glm::vec4& col)
                {
                    if (x < 0) vml::os::Message::Error("PathFinder : ", "x component is less than 0");
                    if (y < 0) vml::os::Message::Error("PathFinder : ", "y component is less than 0");
                    if (x >= Width) vml::os::Message::Error("PathFinder : ", "x component is greater than Width ( %d )", Width);
                    if (y >= Height) vml::os::Message::Error("PathFinder : ", "y component is greater than Width ( %d )", Height);

                    int offset = x + y * Width;

                    if (IdData[offset] == -1)
                    {
                        vml::DebugRender->DrawTriangle(view, Pa[offset], Pb[offset], Pc[offset], vml::colors::Red);
                        vml::DebugRender->DrawTriangle(view, Pb[offset], Pc[offset], Pd[offset], vml::colors::Red);
                    }
                    else
                    {
                        vml::DebugRender->DrawTriangle(view, Pa[offset], Pb[offset], Pc[offset], col);
                        vml::DebugRender->DrawTriangle(view, Pb[offset], Pc[offset], Pd[offset], col);
                    }
                ...
                void DrawCellFromPoint(vml::views::View* view, const glm::vec3& p, const glm::vec4& col)
                {
                    int i = (int)((p.x - BoundingBoxMin.x) / Dw);
                    int j = (int)((p.y - BoundingBoxMin.y) / Dh);
                    if (i < 0 || j < 0 || i >= Width || j >= Height)
                        vml::os::Message::Error("Pathfinder : ", "index out of range");
                    DrawCell(view, i, j, col);
                }
                ...
                void DrawPath(vml::views::View* view)
                {
                    ...
                    for (int i = 0; i < PAvgrOutPathCount - 1; ++i)
                    {
                        glm::vec3 p = PAvgrOut[i];
                        glm::vec3 q = PAvgrOut[i + 1];

                        vml::DebugRender->DrawLine(view, p, q, vml::colors::White, true);

                    }

                    //     DrawBoundingBox(view, vml::colors::White);
				...
				PathFinder(const std::string& filename)
				{
					Width                 =  0;
					Height                =  0;
					Size                  =  0;
					...
					vml::Logger::GetInstance()->Out(vml::Logger::GetInstance()->GetCurrentDate(), "Level", "Loading NavMask", FileName);
				...
				{
					vml::SafeDelete(Data);
                    vml::SafeDelete(IdData);
                    vml::SafeDelete(Distance);
                    vml::SafeDeleteArray(Parent);
                    vml::SafeDeleteArray(Path);
                    vml::SafeDeleteArray(Pa);
                    vml::SafeDeleteArray(Pb);
                    vml::SafeDeleteArray(Pc);
                    vml::SafeDeleteArray(Pd);
                    vml::SafeDeleteArray(PAvgrIn);
                    vml::SafeDeleteArray(PAvgrOut);
                    vml::SafeDeleteArray(PAvgrDir);
                    vml::SafeDeleteArray(PAvgrInvSegmentLength);
                    vml::SafeDeleteArray(PAvgrSegmentLength);
                }
