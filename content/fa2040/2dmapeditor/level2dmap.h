...
#include <vml2.0/content/fa2040/2dmapeditor/router.h>
	...
	namespace tools
	{
		class Level2dMap
		{
			...
			private:
				...
				void Controller(GLFWwindow* glwindow)
				{
					if (!Level2dGenerator)
						return;

					if (glfwGetKeyOnce(glwindow, GLFW_KEY_UP))
					{
					}

					if (glfwGetKeyOnce(glwindow, GLFW_KEY_DOWN))
					{
					}

					if (glfwGetKeyOnce(glwindow, GLFW_KEY_RIGHT))
					{
					}

					if (glfwGetKeyOnce(glwindow, GLFW_KEY_LEFT))
					{
					}

					if (glfwGetKeyOnce(glwindow, GLFW_KEY_PAGE_UP))
					{
					}

					if (glfwGetKeyOnce(glwindow, GLFW_KEY_PAGE_DOWN))
					{
					}
			...
			public:
				...
				void Draw(vml::OpenGLContextWindow* glcontextwindow, GLFWwindow* glfwindow, bool wantcapture)
				{
					// update current view

					View->UpdateView(glcontextwindow);

					// perform contorlling 
					// wantcapture allows to use
					// dearimgui widgets while cursor is
					// over them 

					if (!wantcapture)
					{
						Controller(glfwindow);

						UpdateInput(glcontextwindow);
					}

					// begin rendering

					vml::DebugRender->Begin();
					...
					if ( Level2dGenerator )
					{
						if (ExternalTriangleLayerVisible) Level2dGenerator->ExternalTriangleLayer2.Draw(ProjMat, Texture->GetWidth(), Texture->GetHeight(), vml::colors::Green);
						if (BorderLayerVisible) Level2dGenerator->BorderLayer.Draw(ProjMat, Texture->GetWidth(), Texture->GetHeight(), vml::colors::Blue);
						if (TriangleLayerVisible) Level2dGenerator->TriangleLayer.Draw(ProjMat, Texture->GetWidth(), Texture->GetHeight(), vml::colors::Red);
					...
					vml::DebugRender->DrawCheckeredQuad(View, 150, glm::vec3(0, 0, 0), 64, vml::colors::White * 0.5f, vml::colors::White * 0.525f);
				...
				void LoadTexture(const std::string& filename)
				{
					...
					TextureAspectRatioString = vml::strings::StringConverter::FromIntToString(Texture->GetHeight() / Texture->GetWidth());
				...
				bool CompileAndSaveMap(const std::string& filename)
				{
					if (Level2dGenerator)
					{
						
						// level generatorio accepts, texture data, scaling factor and erosion factor
						...
						Level2dGenerator->Go(GetTexture(),3,3);
