...
namespace fa2040
{
	...
	class Drone : public vml::objects::Object3d_2
	{
		...
		public:
			...
			vml::objects::Model3d_2* LowerBodyModel;
			vml::objects::Model3d_2* SupportShaftModel;
			vml::objects::Model3d_2* Rotor1Model;
			vml::objects::Model3d_2* Rotor2Model;
			vml::objects::Model3d_2* GunPivotModel;
			vml::objects::Model3d_2* GunModel;
			vml::objects::Model3d_2* Blade1Model;
			vml::objects::Model3d_2* Blade2Model;
		//	ObjectFrustm			 Frustum;
			...
			glm::ivec2 GetNavMeshCell(/*vml::views::View* view*/)
			{
				if (!PathFinder)
					vml::os::Message::Error("Drone : ", "PathFinder is nnull");
			...
			void DrawPath(vml::views::View* view)
			{
				...
				//	PathFinder->DrawCell(view, PathA.x, PathA.y, vml::colors::Green);
				//	PathFinder->DrawCell(view, PathB.x, PathB.y, vml::colors::Red);

				for (int i = 0; i < PathCount - 1; ++i)
				{
					vml::DebugRender->DrawLine(view, Path[i], Path[i+1], vml::colors::White, true);
				}

				if (PathFinder)
					PathFinder->DrawBoundingBox(view, vml::colors::White);
			...
			void Controller(vml::views::View* view)
			{
				...
				if (State == START_PATH)
				{
					...
					//	CruiseSpeed   = OriginalCruiseSpeed;
				...
				if (State == START_BACKWARD_PATROL)
				{
					...
					Position.x = Path[PathIndex].x - T * PathDir[j].x;
					Position.y = Path[PathIndex].y - T * PathDir[j].y;
					Position.z = Path[PathIndex].z - T * PathDir[j].z;
					...
					if (T > 1.0f)
					{
						CurPath -= PathSegmentLength[j] / PathLenght;
						PathIndex--;
						T = 0.0f;
				...
				// go back after stopping chase to starting rout to replay initial route
			...
			Drone(vml::geo2d::PathFinder* pathfinder, fa2040::CleoCopter* player, const std::string& screenname, const glm::vec3& pos, const glm::vec3& rot, const glm::vec3& scaling, int priority) : vml::objects::Object3d_2(screenname)
			{
				Player					 =  nullptr;
				PathFinder				 =  nullptr;
				Path					 =  nullptr;
				...
				CreateObject(vml::Session::GetInstance()->GetMainPath(), pos, rot, scaling);
			...
			~Drone()
			{
				...
				vml::SafeDeleteArray(PathDir);
				vml::SafeDeleteArray(PathInvSegmentLength);
				vml::SafeDeleteArray(PathSegmentLength);
			}
