	...
	namespace meshes
	{
		...
		class RemoveDuplicates3D
		{
						...
						int FindVertex(const vml::geo3d::Vertex& vertex, const float eps = 0.000000000001f) const
						{
							for (size_t i = 0; i < VertexArray.size(); ++i)
							{
								if (fabsf(VertexArray[i].Vertex.Pos.x    - vertex.Pos.x)    < eps &&
									...
									fabsf(VertexArray[i].Vertex.Color.r  - vertex.Color.r)  < eps &&
									fabsf(VertexArray[i].Vertex.Color.g  - vertex.Color.g)  < eps &&
									fabsf(VertexArray[i].Vertex.Color.b  - vertex.Color.b)  < eps &&
									fabsf(VertexArray[i].Vertex.Color.a  - vertex.Color.a)  < eps)
								{
			...
			public:
				...
				void Finalize()
				{
					vml::bits32::SetToTrue(Flags, FINALIZED);
				}
