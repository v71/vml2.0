...
namespace vml
{
	namespace textures
	{
		...
		class Texture : public vml::resources::SharedResource
		{
			...
			private:
				...
				void CreateTexture2dFromData()
				{
					...
					if (Data)
					{
						...
						if (PreserveData == TEXTURE_RELEASE_DATA_TRUE )
						{
			...
			public:
				...
				const std::string Dump()
				{
					std::string text = "FileName              : "+ ResourceFileName +"\n";
						text += "Width                 : " + vml::strings::StringConverter::FromIntToString(Width) + "\n";
						text += "Height                : " + vml::strings::StringConverter::FromIntToString(Height) + "\n";
					if ( TextureType== GL_TEXTURE_2D) 
						text += "TextureType           : GL_TEXTURE_2D\n";
					text += "Id                    : " + vml::strings::StringConverter::FromIntToString(ID) + "\n";
					text += "BPP                   : " + vml::strings::StringConverter::FromIntToString(BPP) + "\n";
					if (MipMapsGenerated == TEXTURE_MIPMAP_TRUE) 
						text += "MipMapsGenerated      : Yes\n";
					if (MipMapsGenerated== TEXTURE_MIPMAP_FALSE) 
						text += "MipMapsGenerated      : No\n";
					text += "Anisotropy            : " + vml::strings::StringConverter::FromFloatToString(Anisotropy) + "\n";
					if (Minification == TEXTURE_FILTER_MIN_NEAREST )
					...
					if (RepeatS != TEXTURE_REPEAT_S)
						text += "RepeatU               : TEXTURE_REPEAT_S\n";
					if (RepeatS != TEXTURE_MIRRORED_S)
						text += "RepeatU               : TEXTURE_MIRRORED_S\n";
					if (RepeatS != TEXTURE_CLAMP_TO_EDGE_S)
						text += "RepeatU               : TEXTURE_CLAMP_TO_EDGE_S\n";
					if (RepeatS != TEXTURE_CLAMP_TO_BORDER_S)
						text += "RepeatU               : TEXTURE_CLAMP_TO_BORDER_S\n";
					...
					if (Resident == TEXTURE_RESIDENT_TRUE)
						text += "Texture Resident      : Yes\n";
					if (Resident == TEXTURE_RESIDENT_FALSE)
						text += "Texture Resident      : False\n";
					...
					if (PreserveData == TEXTURE_RELEASE_DATA_FALSE)
						text += "Texture Data Released : False\n";
				    text += "Instances             : "+vml::strings::StringConverter::FromIntToString(GetInstancesCount())+"\n";
					return text;
				}
				...
				constexpr bool		    IsDataPreserved() const { return PreserveData; }

				// ---------------------------------------------------------
				// ctor / dtor

				Texture( const std::string &resourcefilename,
						 const int repeats	        = TEXTURE_CLAMP_TO_EDGE_S,
						 const int repeatt	        = TEXTURE_CLAMP_TO_EDGE_T,
						 const int magnification    = TEXTURE_FILTER_MAG_LINEAR,
						 const int minification     = TEXTURE_FILTER_MIN_LINEAR,
						 const int mipmapsgenerated = TEXTURE_MIPMAP_TRUE,
						 const int gammacorrection  = TEXTURE_GAMMA_FALSE,
						 const int preservedata     = TEXTURE_RELEASE_DATA_TRUE,
						 const int resident			= TEXTURE_RESIDENT_FALSE) : vml::resources::SharedResource(resourcefilename)
				{
					...
					MipMapsGenerated = mipmapsgenerated;
					Resident		 = resident;
				...
				~Texture()
				{
					...
					// checks if data is valid then delete it

					vml::SafeDelete(Data);

					vml::Logger::GetInstance()->Out(vml::Logger::GetInstance()->GetCurrentDate(), "Texture", "Releasing instance of Texture", ResourceFileName.c_str());
				}
